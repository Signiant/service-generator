...
var bitbucket = require('./tasks/bitbucket/bitbucket');
...
module.exports = function (io, rootDir){
    ...
    io.on('connection', function(socket) {
        ...
        function repoTask(data){
            ...
            bitbucket(data.name, function(err, response){
                if(err){
                    console.log(err);
                    socket.emit('git:error', err);
                }else{
                    service.repo = response.web;
                    service.sshRepo = response.git;
        ...
        function jenkinsTask(jobName){
            jenkins.run(service.type, jobName, service.sshRepo, function(err, data){
                if(err){
                    console.log(err);
